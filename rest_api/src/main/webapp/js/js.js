$(document).ready(function () {
    ...
    if ($("table.sortable_default").length > 0) {
        $("table.sortable_default").dataTable({iDisplayLength:5, sPaginationType:"full_numbers", bLengthChange:false, bFilter:false, bInfo:false, bPaginate:true, aoColumns:[
            {bSortable:false},
            null,
            null,
            null,
    ...
    if ($("table.sortable").length > 0) {
        $("table.sortable").dataTable({iDisplayLength:5, aLengthMenu:[5, 10, 25, 50, 100], sPaginationType:"full_numbers", aoColumns:[
            {bSortable:false},
            null,
            null,
            null,
    ...
    if ($("textarea.tmce").length > 0) {
        tinymce.init({selector:"textarea.tmce", height:400})
    }
    if ($("textarea.stmce").length > 0) {
        tinymce.init({selector:"textarea.stmce", toolbar:"bold italic | alignleft aligncenter alignright alignjustify | undo redo", menu:[], height:200})
    }
    ...
    if ($("#wizard_validate").length > 0) {
        ...
        $("#wizard_validate").stepy({back:function (d) {
        }, next:function (d) {
    ...
    if ($("#calendar").length > 0) {
        ...
        var c = $("#calendar").fullCalendar({header:{left:"prev,next today", center:"title", right:"month,agendaWeek,agendaDay"}, editable:true, events:[
            {title:"All Day Event", start:new Date(f, a, 1)},
            {title:"Long Event", start:new Date(f, a, e - 5), end:new Date(f, a, e - 2)},
            {id:999, title:"Repeating Event", start:new Date(f, a, e - 3, 16, 0), allDay:false},
            {id:999, title:"Repeating Event", start:new Date(f, a, e + 4, 16, 0), allDay:false},
            {title:"Meeting", start:new Date(f, a, e, 10, 30), allDay:false},
            {title:"Lunch", start:new Date(f, a, e, 12, 0), end:new Date(f, a, e, 14, 0), allDay:false},
            {title:"Birthday Party", start:new Date(f, a, e + 1, 19, 0), end:new Date(f, a, e + 1, 22, 30), allDay:false},
            {title:"Click for Google", start:new Date(f, a, 28), end:new Date(f, a, 29), url:"http://google.com/"}
        ], droppable:true, selectable:true, selectHelper:true, select:function (j, d, g) {
            var h = prompt("Event Title:");
            if (h) {
                c.fullCalendar("renderEvent", {title:h, start:j, end:d, allDay:g}, true)
            }
    ...
    if ($("#price_rage").length > 0) {
        $("#price_rage").slider({range:true, min:0, max:3000, values:[1000, 2000], slide:function (d, g) {
...
$(document).ready(function () {
    if ($("#dash_chart_1").length > 0) {
        ...
        var c = $.plot($("#dash_chart_1"), [
            {data:u, label:"Sales"}
        ], {series:{lines:{show:true}, points:{show:true}}, grid:{hoverable:true, clickable:true, margin:{left:110}}, xaxis:{ticks:[
            [1, "Mon"],
            [2, "Tue"],
            [3, "Wed"],
            [4, "Thu"],
    ...
    if ($("#dash_chart_2").length > 0) {
        ...
        var b = $.plot($("#dash_chart_2"), [
            {data:k, label:"Search Traffic"},
            {data:h, label:"Referal Traffic"}
        ], {series:{lines:{show:true}, points:{show:true}}, grid:{hoverable:true, clickable:true}, xaxis:{max:7, ticks:[
            [1, "Mon"],
            [2, "Tue"],
            [3, "Wed"],
            [4, "Thu"],
    ...
    if ($("#chart_line_1").length > 0) {
        ...
        var t = $.plot($("#chart_line_1"), [
            ...
            {data:d, label:"cos(x)"},
            {data:o, label:"sin(y)"}
        ], {series:{lines:{show:true}, points:{show:true}}, grid:{hoverable:true, clickable:true}, yaxis:{min:-1.1, max:1.1}})
    }
    if ($("#chart_bar_1").length > 0) {
        ...
        var q = $.plot($("#chart_bar_1"), [
            ...
            {data:h, label:"Data 2"},
            {data:f, label:"Data 3"}
        ], {series:{stack:0, bars:{show:true, barWidth:0.8, align:"center"}}, grid:{hoverable:true, clickable:true}})
    }
    if ($("#chart_bar_2").length > 0) {
        var p = $.plot($("#chart_bar_2"), [
            ...
            {data:h, label:"Data 2"},
            {data:f, label:"Data 3"}
        ], {series:{stack:0, lines:{show:true, fill:true}, bars:{show:false}}, grid:{hoverable:true, clickable:true}})
    }
    if ($("#chart_pie_1").length > 0) {
        var u = [];
        u[0] = {label:"Data 1", data:40};
        u[1] = {label:"Data 2", data:30};
        u[2] = {label:"Data 3", data:30};
        $.plot("#chart_pie_1", u, {series:{pie:{radius:1, show:true, label:{show:true, radius:2 / 3, formatter:j, threshold:0.1}}}, legend:{show:false}})
    }
    ...
    if ($("#chart_series_onoff").length > 0) {
        ...
        function m() {
            ...
            if (v.length > 0) {
                $.plot("#chart_series_onoff", v, {yaxis:{min:0}, xaxis:{tickDecimals:0}})
            }
        }
    ...
    function e(v, z, w) {
        $('<div class="ftooltip">' + w + "</div>").css({position:"absolute", "z-index":"10", display:"none", top:z - 20, left:v, padding:"3px", "background-color":"rgba(0,0,0,0.5)", "font-size":"11px", "border-radius":"3px", color:"#FFF"}).appendTo("body").fadeIn(200)
    }
    ...
    $("#chart_bar_1,#chart_bar_2,#chart_line_1,#dash_chart_1,#dash_chart_2,#chart_user_1").bind("plothover", function (z, B, w) {
        ...
        if (w) {
            if (r != w.dataIndex) {
                r = w.dataIndex;
                $(".ftooltip").remove();
                var v = w.datapoint[0].toFixed(2), A = w.datapoint[1].toFixed(2);
...
$(document).ready(function () {
    if ($("#google_kyiv_map").length > 0) {
        ...
        var r = {zoom:10, center:e, mapTypeId:google.maps.MapTypeId.ROADMAP};
        var b = new google.maps.Map(document.getElementById("google_kyiv_map"), r);
        var d = new google.maps.LatLng(50.43, 30.6);
        var h = new google.maps.Marker({position:d, map:b, title:"Office title"})
    }
    if ($("#google_world_map").length > 0) {
        var n = new google.maps.LatLng(50, 0);
        var o = {zoom:1, center:n, mapTypeId:google.maps.MapTypeId.ROADMAP};
        var q = new google.maps.Map(document.getElementById("google_world_map"), o);
        var c = new google.maps.LatLng(50, 10);
        var j = {zoom:3, center:c, mapTypeId:google.maps.MapTypeId.ROADMAP};
        var m = new google.maps.Map(document.getElementById("google_eu_map"), j);
        var l = new google.maps.LatLng(50.43, 30.6);
        var a = {zoom:8, center:l, mapTypeId:google.maps.MapTypeId.ROADMAP};
        var p = new google.maps.Map(document.getElementById("google_ptm_map"), a);
        var g = new google.maps.LatLng(50.37, 30.65);
        var f = new google.maps.Marker({position:g, map:p, title:"Marker 1"});
        g = new google.maps.LatLng(50.5, 30.55);
        f = new google.maps.Marker({position:g, map:p, title:"Marker 2"});
        g = new google.maps.LatLng(50.8, 30.55);
        f = new google.maps.Marker({position:g, map:p, title:"Marker 3"});
        function k() {
            var u = new google.maps.Map(document.getElementById("google_search_map"), {mapTypeId:google.maps.MapTypeId.ROADMAP});
            ...
            google.maps.event.addListener(w, "places_changed", function () {
                ...
                for (var A = 0, x; x = z[A]; A++) {
                    var C = {url:x.icon, size:new google.maps.Size(71, 71), origin:new google.maps.Point(0, 0), anchor:new google.maps.Point(17, 34), scaledSize:new google.maps.Size(25, 25)};
                    var y = new google.maps.Marker({map:u, icon:C, title:x.name, position:x.geometry.location});
                    v.push(y);
